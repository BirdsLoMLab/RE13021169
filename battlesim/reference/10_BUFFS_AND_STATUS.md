...
> Complete buff system reference: 80 buff classes, 46 named + 34 data-only buff groups (76 total), mutex rules, full damage pipeline, and per-class parameter documentation.
...
| 2 | type | Duration: 0=instant (execute+destroy), 1+=persistent |
| 3 | group | BuffGroupType for runtime queries via `getBuffByGroup()` |
| 4 | icon | UI icon asset |
| 5 | effect | Visual effect on target |
| 6 | effect_mirror | Mirrored visual effect |
| 7 | mutex | Stacking rule (see below) |
| 8 | add_max | Max stacks (for mutex=3) |
| 9 | bind | BindType (1=bp_lead, 2=bp_bottom, 3=bp_top) |
| 10 | action | Handler class key (into buffMap, 80 classes) |
| 11-15 | param1-5 | Class-specific parameters (see per-class docs below) |

**No XOR encoding** — ConfigBuff has `usesConfigKey: false`. All field values are plain.

### Buff Creation Flow
```
1. addBuff(target, buffId, duration, skillPar)        [line 431489]
2. ConfigBuff lookup → get action, group, mutex, params
3. IGNORE_BUFFIDS check → skip if target blocks this buff (param5 array)
...
7. Set skillPar, duration (with CONTROL_RES / shield_time_extra adjustments)
8. type==0: execute start() + destroy() immediately (instant buffs)
9. type>0: add to BuffCtr, manage lifecycle (persistent buffs)
10. Mutex 3: stack, enforce add_max; refresh existing durations
...
The `mutex` field (index 7) controls what happens when a buff is applied and the same buff ID already exists:

| Mutex | Name | Behavior |
|-------|------|----------|
| 1 | Replace | Stop all existing instances of this buff ID, add new |
| 2 | Unique | Reject new if any instance with this ID exists |
| 3 | Stack w/ Max | Multiple up to `add_max`; refresh all durations; remove oldest when exceeded |
| 4 | Unique/caster | One per caster; reject re-apply from same caster |
| 5 | Refresh/caster | One per caster; reset existing duration instead of rejecting |

**Pre-mutex checks** (applied before mutex logic):
- **Control immunity**: If target has `notControlled` or `invincible`, control-type buffs (dizz, ban_skill, throw_hit, bound, ban_act) are skipped entirely
...
## Data-Only Buff Groups (34 Unnamed)

The BuffGroupType enum defines 46 named groups (above). However, Buff.json data uses **76 unique group values** — 34 groups exist only in data with no named enum entry. These are used for filtering, categorization, and conditional logic.
...
**Tiered Freeze Escalation (Groups 271-279):** Forms a freeze severity ladder — group 271 uses 2s freeze + compounding attribute debuffs, groups 274-276 use 2.5s freeze, and groups 277-279 use 3s freeze. Repeated freezes become progressively more punishing.

**Unit Summoning (Group 5):** Largest data-only group (53 buffs) covers the entire summon system: `call_unit` (basic), `call_spirit` (spirit), and `copy_unit` (clone/mirror). Some include complex attribute inheritance arrays in param5.

**Conditional Logic (Group 504):** Uses the `check` action to evaluate conditions before applying effects. References another buff ID and an interval, creating branching buff logic.
...
| 10 | Attribute | Uses param2 attr_id, param3 for target/caster selection | No |
...
#### BuffDizz Parameters
- param1 = dizzType (sub-type; 0 = standard stun affected by CONTROL_RES)
...
```
dizz, ban_skill, throw_hit, bound, ban_act
```
...
| BuffNotControlled | `not_controll` | — | — | Control immunity |
...
- **46** named BuffGroupType categories + **34** data-only groups = **76** total
